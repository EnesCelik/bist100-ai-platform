#!/bin/zsh
set -euo pipefail
cd /Users/ahmetenescelik/Developer/bist100-ai-platform
open -a Docker >/dev/null 2>&1 || true
/usr/local/bin/docker compose up -d postgres >/tmp/bist100-docker.log 2>&1 || /opt/homebrew/bin/docker compose up -d postgres >/tmp/bist100-docker.log 2>&1 || docker compose up -d postgres >/tmp/bist100-docker.log 2>&1
pkill -f "uvicorn app.api.main:app" >/dev/null 2>&1 || true
nohup .venv/bin/python -m uvicorn app.api.main:app --host 127.0.0.1 --port 8000 > /tmp/bist100-ai-platform.log 2>&1 &
sleep 3
curl -s http://127.0.0.1:8000/api/v1/db/health
