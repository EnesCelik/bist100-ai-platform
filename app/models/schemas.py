...
class RuntimeHealthResponse(BaseModel):
    ...
    global_news_watch_enabled: bool = Field(default=False, description="Whether global news watch job is enabled")
    ...
    last_global_news_watch_started_at: str | None = Field(default=None, description="Global-news watch job last start time")
    last_global_news_watch_completed_at: str | None = Field(default=None, description="Global-news watch job last completion time")
    last_global_news_watch_status: str | None = Field(default=None, description="Global-news watch job last status")
    last_global_news_watch_message: str | None = Field(default=None, description="Global-news watch job last summary message")
    ...
    token_refresh_enabled: bool = Field(default=False, description="Whether the MarketDataToken auto-refresh job is enabled")
    last_token_refresh_started_at: str | None = Field(default=None, description="Token-refresh job last start time")
    last_token_refresh_completed_at: str | None = Field(default=None, description="Token-refresh job last completion time")
    last_token_refresh_status: str | None = Field(default=None, description="Token-refresh job last status")
    last_token_refresh_message: str | None = Field(default=None, description="Token-refresh job last summary message")
    policy_rate_watch_enabled: bool = Field(default=False, description="Whether the TCMB policy rate watch job is enabled")
    last_policy_rate_watch_started_at: str | None = Field(default=None, description="Policy rate watch job last start time")
    last_policy_rate_watch_completed_at: str | None = Field(default=None, description="Policy rate watch job last completion time")
    last_policy_rate_watch_status: str | None = Field(default=None, description="Policy rate watch job last status")
    last_policy_rate_watch_message: str | None = Field(default=None, description="Policy rate watch job last summary message")
    real_rate_watch_enabled: bool = Field(default=False, description="Whether the TCMB real rate watch job is enabled")
    last_real_rate_watch_started_at: str | None = Field(default=None, description="Real rate watch job last start time")
    last_real_rate_watch_completed_at: str | None = Field(default=None, description="Real rate watch job last completion time")
    last_real_rate_watch_status: str | None = Field(default=None, description="Real rate watch job last status")
    last_real_rate_watch_message: str | None = Field(default=None, description="Real rate watch job last summary message")
    kap_disclosure_watch_enabled: bool = Field(default=False, description="Whether the KAP disclosure watch job is enabled")
    last_kap_disclosure_watch_started_at: str | None = Field(default=None, description="KAP disclosure watch job last start time")
...
class PaperDecisionPerformanceSummaryResponse(BaseModel):
    ...
    resolved_count: int = Field(description="Logs that produced a non-pending outcome (win+loss+mixed+open)", examples=[8])
    decided_count: int = Field(description="Logs with a definitive win/loss/mixed outcome (excludes still-open, undecided logs)", examples=[5])
    ...
    win_rate: float | None = Field(default=None, description="Share of decided (win/loss/mixed) logs ending in win; still-open logs are excluded from the denominator")
    loss_rate: float | None = Field(default=None, description="Share of decided (win/loss/mixed) logs ending in loss; still-open logs are excluded from the denominator")
...
class PaperDecisionResolvedPerformanceSummaryResponse(BaseModel):
    ...
    open_count: int = Field(description="Resolved outcomes labeled open (evaluated but no TP/SL hit yet - not a decided outcome)", examples=[2])
    decided_count: int = Field(description="Resolved outcomes with a definitive win/loss/mixed label (excludes open)", examples=[3])
    resolved_win_rate: float | None = Field(default=None, description="Win share among decided (win/loss/mixed) outcomes; open outcomes are excluded from the denominator")
    resolved_loss_rate: float | None = Field(default=None, description="Loss share among decided (win/loss/mixed) outcomes; open outcomes are excluded from the denominator")
...
class PaperTradeItem(BaseModel):
    ...
    profit_protected: bool = Field(description="Whether profit protection rule has activated")
...
class PaperTradeDailyReportResponse(BaseModel):
    ...
    profit_protected_count: int = Field(description="Trades where profit protection activated")
...
class PaperTradeAllTimeSummaryResponse(BaseModel):
    strategy_name: str | None = Field(default=None, description="Optional strategy filter")
    total_trades: int = Field(description="All-time number of paper trades (open + closed)", examples=[87])
    open_count: int = Field(description="Currently open trades", examples=[3])
    closed_count: int = Field(description="Closed trades", examples=[84])
    win_count: int = Field(description="Closed trades with outcome=win", examples=[40])
    loss_count: int = Field(description="Closed trades with outcome=loss", examples=[15])
    neutral_count: int = Field(description="Closed trades with outcome=neutral", examples=[20])
    no_fill_count: int = Field(description="Closed trades that never actually filled", examples=[9])
    decided_count: int = Field(description="Closed trades with a definitive win/loss/neutral outcome (excludes no_fill)", examples=[75])
    win_rate: float | None = Field(default=None, description="Win share among decided (win/loss/neutral) trades")
    total_capital_allocated_try: float = Field(description="Sum of capital_allocated across all trades, in TRY", examples=[125000.0])
    total_realized_pnl_try: float = Field(description="Sum of realized PnL across all trades, in TRY", examples=[3200.5])
    total_unrealized_pnl_try: float = Field(description="Sum of unrealized PnL on currently open trades, in TRY", examples=[450.0])
    total_pnl_try: float = Field(description="Realized + unrealized PnL across all trades, in TRY", examples=[3650.5])
    total_pnl_percent: float | None = Field(default=None, description="total_pnl_try as a percent of total_capital_allocated_try")
    best_ticker: str | None = Field(default=None, description="Ticker with the highest total PnL")
    best_pnl_try: float | None = Field(default=None, description="Highest total PnL in TRY")
    worst_ticker: str | None = Field(default=None, description="Ticker with the lowest total PnL")
    worst_pnl_try: float | None = Field(default=None, description="Lowest total PnL in TRY")
    summary: str = Field(description="Human-readable all-time performance summary")


class LlmTraceItem(BaseModel):
    timestamp: str = Field(description="Call timestamp (ISO 8601)")
    ticker: str = Field(description="Ticker the call was made for", examples=["GARAN"])
    model: str = Field(description="Claude model id used", examples=["claude-opus-5"])
    status: str = Field(description="ok, error, or empty_response", examples=["ok"])
    input_tokens: int | None = Field(default=None, description="Input tokens for this call")
    output_tokens: int | None = Field(default=None, description="Output tokens for this call")
    estimated_cost_usd: float | None = Field(default=None, description="Estimated cost in USD for this call")
    latency_ms: float | None = Field(default=None, description="Call latency in milliseconds")
    error: str | None = Field(default=None, description="Error message when status is not ok")


class LlmTraceSummaryResponse(BaseModel):
    total_calls: int = Field(description="Total LLM synthesis calls logged", examples=[12])
    ok_count: int = Field(description="Calls that returned usable text", examples=[11])
    error_count: int = Field(description="Calls that failed or returned nothing usable", examples=[1])
    total_input_tokens: int = Field(description="Sum of input tokens across ok calls", examples=[5400])
    total_output_tokens: int = Field(description="Sum of output tokens across ok calls", examples=[3200])
    total_estimated_cost_usd: float = Field(description="Sum of estimated cost across ok calls, in USD", examples=[0.14])
    average_latency_ms: float | None = Field(default=None, description="Average latency across ok calls, in milliseconds")
    recent_traces: list[LlmTraceItem] = Field(default_factory=list, description="Most recent trace entries, newest first")
    summary: str = Field(description="Human-readable trace summary")
